...
app.Map("/api/quiz/{**catch-all}", async context =>
{
	...
	var path = context.Request.Path.Value?.Replace("/api/quiz", "/api/quiz");
...
// Configure routing to ConceptMap endpoints
app.Map("/api/conceptmap/{**catch-all}", async context =>
{
	var quizServiceUrl = "http://conceptnavigator-quiz-service";
	var path = context.Request.Path.Value?.Replace("/api/conceptmap", "/api/conceptmap");
	var query = context.Request.QueryString.Value;
	var url = $"{quizServiceUrl}{path}{query}";
